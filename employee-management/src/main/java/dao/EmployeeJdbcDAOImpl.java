...
public class EmployeeJdbcDAOImpl implements EmployeeDAO {
	private static final Logger logger = LoggerFactory.getLogger(EmployeeJdbcDAOImpl.class);
	private Connection getConnection() throws SQLException {
		return DatabaseUtil.getConnection();
	}

	public EmployeeJdbcDAOImpl() throws EmployeeDataAccessException {
		try {
			ensureDefaultAdmin();
		} catch (SQLException e) {
			throw new EmployeeDataAccessException("Failed to initialize JDBC DAO", e);
		}
	}
	...
	private void ensureDefaultAdmin() throws SQLException {
		...
		try (Connection conn = getConnection()) {
			...
			try {
				...
				logger.info("Default admin created successfully with id={}", adminId); 
			} catch (SQLException e) {
				conn.rollback();
	...
	public String addEmployee(String name, String dept, String address, String email, List<String> roles,
			String hashPassword) throws EmployeeDataAccessException {
		try (Connection conn = getConnection()) {
			conn.setAutoCommit(false);
			int generatedId;
			try {
				...
				conn.rollback();
				logger.error("Add employee failed", e);
				throw new EmployeeDataAccessException("Add employee failed", e);
			} finally {
				conn.setAutoCommit(true);
			}
		} catch (SQLException e) {
			throw new EmployeeDataAccessException("Database connection failed", e);
		}
	}
	...
	public void updateEmployee(String id, String name, String dept, String address, String email)
		...
		try (Connection conn = getConnection()) {
			conn.setAutoCommit(false);
			try (PreparedStatement ps = conn
					.prepareStatement("update employees set name=?, department=?, address=?, email=? where emp_id=?")) {
				ps.setString(1, name);
				ps.setString(2, dept);
				ps.setString(3, address);
				ps.setString(4, email);
				...
				conn.rollback();
				logger.error("Update employee failed for empId={}", id, e);
				throw new EmployeeDataAccessException("Update employee failed", e);
			} finally {
				conn.setAutoCommit(true);
			}
		} catch (SQLException e) {
	...
	public void deleteEmployee(String id)
	    ...
	    try (Connection conn = getConnection()) {
	        ...
	        try (PreparedStatement ps =
	                     conn.prepareStatement("UPDATE employees SET is_deleted = true WHERE emp_id = ? AND is_deleted = false")) {
	            ps.setString(1, id);
	            int rowsUpdated = ps.executeUpdate();
	            if (rowsUpdated == 0) {
	                conn.rollback();
	            ...
	            conn.rollback();
	            logger.error("Delete employee failed for empId={}", id, e);
	            throw new EmployeeDataAccessException("Delete employee failed", e);
	        } finally {
	            conn.setAutoCommit(true);
	        }
	    } catch (SQLException e) {
	...
	public void changePassword(String id, String password)
		...
		try (Connection conn = getConnection()) {
			...
			try (PreparedStatement ps = conn
				...
				conn.rollback();
				logger.error("Change password failed for empId={}", id, e);
				throw new EmployeeDataAccessException("Change password failed", e);
			} finally {
				conn.setAutoCommit(true);
			}
		} catch (SQLException e) {
	...
	public void resetPassword(String id, String password)
		...
		try (Connection conn = getConnection()) {
			...
			try (PreparedStatement ps = conn
				...
				conn.rollback();
				logger.error("Change password failed for empId={}", id, e);
				throw new EmployeeDataAccessException("Reset password failed", e);
			} finally {
				conn.setAutoCommit(true);
			}
		} catch (SQLException e) {
	...
	public void grantRole(String id, String role)
		...
		try (Connection conn = getConnection()) {
			...
			try {
				...
				conn.rollback();
				logger.error("Grant role failed for empId={}", id, e); 
				throw new EmployeeDataAccessException("Grant role failed", e);
			} finally {
				conn.setAutoCommit(true);
			}
		} catch (SQLException e) {
	...
	public void revokeRole(String id, String role)
		...
		try (Connection conn = getConnection()) {
			...
			try {
				...
				int count;
				try (PreparedStatement ps = conn
						.prepareStatement("select count(*) from employee_roles where emp_id=?")) {
					ps.setString(1, id);
					try (ResultSet rs = ps.executeQuery()) {
						rs.next();
						count = rs.getInt(1);
				...
				conn.rollback(); 
				logger.error("Revoke role failed for empId={}", id, e);
				throw new EmployeeDataAccessException("Revoke role failed", e);
			} finally {
				conn.setAutoCommit(true);
			}
		} catch (SQLException e) {
	...
	@Override
	public Employee getEmployeeById(String id) throws EmployeeNotFoundException, EmployeeDataAccessException {
		try (Connection conn = getConnection();
				PreparedStatement ps = conn.prepareStatement(
						"select e.emp_id, e.name, e.department, e.address, e.email, " + "l.password, l.first_login "
								+ "from employees e join employee_login l on e.emp_id = l.emp_id "
								+ "where e.emp_id = ? and e.is_deleted=false")) {
			ps.setString(1, id);
			try (ResultSet rs = ps.executeQuery()) {
				if (!rs.next())
					throw new EmployeeNotFoundException("Employee not found");
			...
			logger.error("Fetch employee failed for empId={}", id, e);
			throw new EmployeeDataAccessException("Fetch employee failed", e);
		}
	}

	@Override
	public List<Employee> getAllEmployees() throws EmployeeDataAccessException {
		...
		try (Connection conn = getConnection();
			...
			logger.error("Fetch all employees failed", e);
			throw new EmployeeDataAccessException("Fetch all employees failed", e);
		}
	}
}
